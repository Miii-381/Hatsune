...
public:
	...
	void on_enter(bool is_debug)
	{
		...
		// 获取歌曲时长，确定歌曲该在何时结束
		song_length = atoi(origin_info.song_length);
		...
		if (!game_pause)
		{
			...
			else
			{
				...
				if (!game_end && transision_complete)
					game_time += delta;		// 游戏时间累加，用于判定note和游戏是否结束
			...
			// 歌曲结束判定
			if (game_time >= song_length + 1000 && game_end == false)
			{
				game_start = false;
				game_end = true;
				point_display.clear();
	...
	void on_draw(bool is_debug)
	{
		...
		if (game_start)
		{
			printf_s("game_time:%d", game_time);
	...
	void load_beatmap()
	{
		...
		while (getline(file, line))
		{
			...
			if(type == 1)
				note_storage[ID - 1].push_back(new Click(numToPosition(pos), start_time - ID * 13, ID, type));
			else if(type == 2)
				note_storage[ID - 1].push_back(new Hold(numToPosition(pos), start_time - ID * 13, end_time - ID * 13 - 13, ID, type));
		}
	}
...
private: // note
	...
	DWORD song_length;				// 歌曲长度，决定何时跳转结算场景
